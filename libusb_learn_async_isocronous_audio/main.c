...
void LIBUSB_CALL write_callback(struct libusb_transfer *t)
{
    ...
    {
        memcpy(t->buffer, lookupTable, t->length);

        //to get an concrete address inside buffer (with an offset) - use the following function:
           // unsigned char *buf = libusb_get_iso_packet_buffer_simple(t, 0);
