...
/*NOTE:
This host application blinks a LED connected on a host STM32F103C8 to PB0 (TIM3 CH3 PWM)
Use the "stm32_usb_interrrupt_host_to_dev" folder with host software to run the current application.
*/
    ...
    if (write_result == 0)
        ...
        */
        ...
        while (1) {
                ...
                //a host sends 4 bytes to a device - as wrote in the device descriptor
                  usb_write_async(0x01, (unsigned char*)&anyData, 4);
                //When a semaphore not released, the OS kernel stop execution
                //and reassign CPU to another tasks.When a semaphore released - the Kernel wakes up
                  wait_for_write();
                // the main thread and execution continue from the next code line
